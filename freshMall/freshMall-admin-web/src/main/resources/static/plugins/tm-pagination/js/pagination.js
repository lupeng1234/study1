...
angular.module('pagination', []).directive('tmPagination', [function() {
	return {
		...
		template: 
			'<div class="page-list">'+
			'	<ul class="pagination pagination-sm" ng-show="conf.totalItems > 0">'+
			...
			'		<li ng-class="{disabled: conf.currentPage == conf.numberOfPages}" ng-click="nextPage()" class="disabled"><span>下一页</span></li>'+
			'	</ul>'+
			...
			'		共<strong>{{ conf.totalItems }}</strong>条记录'+
			'	</div>'+
			'	<div class="no-items" ng-show="conf.totalItems <= 0"><strong>暂无数据记录！</strong></div>'
			,
		replace: true,
		scope: {
			conf: '='
